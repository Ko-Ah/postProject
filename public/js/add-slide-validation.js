$(document).ready(function () {
  ...
  function validation(event) {
    ...
    if (subtitle.length > 0) {
      if (subtitle.val() == "") {
        event.preventDefault();
        subtitle.next(".errors").text("* فیلد زیرعنوان نمی تواند خالی باشد!");
      } else if (regSpecial.test(subtitle.val())) {
        subtitle.next(".errors").text("* زیرعنوان وارد شده معتبر نیست!");
      } else {
    ...
    if (image.length > 0) {
      if (image.val() == "") {
        event.preventDefault();
        image.next(".errors").text("* فیلد عکس نمی تواند خالی باشد!");
      } else {
